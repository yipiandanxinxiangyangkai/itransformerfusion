...
class TextStockFusionModel(nn.Module):
    def __init__(self, configs):
        ...
        # [!! 新增 !!] Bi-LSTM 文本编码器
        # 假设 configs.text_hidden_dim = configs.d_model // 2
        # (例如: d_model=64, text_hidden_dim=32)
        if (configs.d_model // 2) * 2 != configs.d_model:
            print(f"警告: d_model ({configs.d_model}) 不是 text_hidden_dim ({configs.text_hidden_dim}) 的两倍。")
            print("这可能导致维度不匹配。请确保 d_model = text_hidden_dim * 2")

        self.text_encoder = nn.LSTM(
            input_size=configs.d_model,          # 输入特征维度 (e.g., 64)
            hidden_size=configs.text_hidden_dim, # LSTM 隐藏层大小 (e.g., 32)
            num_layers=1,                        # LSTM 层数
            batch_first=True,                    # 保证输入形状是 [Batch, Seq, Feature]
    ...
    def forward(self, x_enc, x_mark_enc, text_ids_enc, x_dec=None, x_mark_dec=None):
        ...
        # text_vec_flat 形状: [B*L_stock, L_text, d_model] (e.g., [120, 50, 64])

        # --- [!! 修改 !!] ---
        # 使用 Bi-LSTM 替换 torch.mean
        
        # _ (lstm_output) 形状: [120, 50, hidden_size*2] (e.g., [120, 50, 64])
        # h_n (final hidden) 形状: [num_layers*2, 120, hidden_size] (e.g., [2, 120, 32])
        _ , (h_n, c_n) = self.text_encoder(text_vec_flat)
        
        # 我们需要 h_n。它包含了前向 (h_n[0]) 和后向 (h_n[1]) 的最后状态
        # 1. 调整维度: [2, 120, 32] -> [120, 2, 32]
        h_n = h_n.permute(1, 0, 2)
        
        # 2. 拼接前向和后向: [120, 2, 32] -> [120, 64]
        day_text_vec = h_n.contiguous().view(B * self.stock_seq_len, -1) 
